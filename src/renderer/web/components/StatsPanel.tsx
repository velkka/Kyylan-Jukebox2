...
export default function StatsPanel({ onError }: { onError: (msg: string) => void }): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {tab === 'played' && (
        <Ranking items={stats?.topPlayed ?? []} unit="play" empty="Nothing played yet." />
      )}
      {tab === 'downvoted' && (
        <Ranking items={stats?.topDownvoted ?? []} unit="downvote" empty="No downvotes yet." />
      )}
      {tab === 'guests' && (
        <Guests items={stats?.users ?? []} onChange={load} onError={onError} isAdmin={isAdmin} />
      )}
    </Group>
  )
}
        ...
        <li key={h.id} className="flex items-center gap-2 text-sm">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-white/45">
              ...
              {' · '}
              {h.source === 'random' ? (
                <span className="text-white/35">random pick</span>
              ) : (
